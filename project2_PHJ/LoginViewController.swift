...
class LoginViewController: UIViewController,UITextFieldDelegate {
    ...
    @IBAction func loginPressed() {
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/M07/login/loginUser.php"
            ...
            do {
                ...
                if success == "YES" {
                    if let name = jsonData["name"] as? String { DispatchQueue.main.async {
                        self.labelStatus.text = name + "님 안녕하세요?" // 아래로 새로 추가된 코드
                        let appDelegate = UIApplication.shared.delegate as! AppDelegate
                        appDelegate.ID = self.loginUserid.text
                        appDelegate.userName = name // 여기까지 추가된 코드 (performSegue 전에 해줘야함)
